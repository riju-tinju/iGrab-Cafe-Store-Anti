    ...
    getContacts: (req, res) => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // TEMPORARILY DISABLED: Filter by admin's selected branch
                const adminBranch = req.session?.admin?.selectedBranch;
                console.log('Admin selected branch:', adminBranch);
                console.log('Full session:', req.session?.admin);
                ...
                console.log('SHOWING ALL CONTACTS (branch filter disabled)');
                ...
                // Get status counts (all contacts, branch filter disabled)
